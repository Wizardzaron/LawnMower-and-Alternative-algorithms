...
class TestFailureException 
{
  public:
    ...
    TestFailureException(int line, const std::string file, const std::string& message): _line(line),_file(file),_message(message) 
        { 
...
class RubricCriterion 
{
  public:
    ...
    // below.
    RubricCriterion(const std::string& name, int points, std::function<void()> test): _name(name), _points(points), _test(test)
  { 
    assert(points > 0); 
  }
  ...
  const std::function<void()>& test() const 
  { 
    return _test; 
  }
...
public:
  ...
  int run() 
  {
    ...
    // print summary score
    std::cout << "TOTAL SCORE = " << earned_points << " / " << total_points<< std::endl<< std::endl;
